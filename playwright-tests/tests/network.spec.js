...
test("wallet should use default network (mainnet) when network value not provided", async ({
  page,
}) => {
  await page.goto("/");

  await page.getByRole("button", { name: "Open wallet selector" }).click();

  await page.locator("#module-my-near-wallet").click();

  await page.waitForURL('https://app.mynearwallet.com/**');
});

test("wallet should use testnet network when network attribute is provided", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page.waitForURL('https://testnet.mynearwallet.com/**');
});
